...
;(async()=>{
 for(const id of ['tiger','churchill','elefant']){
  const v=VEHICLES.find(x=>x.id===id)!; let meshes:any=null
  ...
  const t0=Date.now()
  buildCamoExclusionMask(meshes, v.id, v.faction as any)
  console.log(`${id.padEnd(12)} ${meshes.length} meshes -> buildCamoExclusionMask ${Date.now()-t0} ms`)
 }
})()
